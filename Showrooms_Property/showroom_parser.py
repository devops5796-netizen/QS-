...
# property runs on a different backend domain than the rest of qatarsale
BASE_URL = "https://qatarsale.com"
API_BASE = "https://production-api.yousale.com/api"
...
# SHOWROOM LIST
# -------------------------
def get_showroom_links() -> list[str]:
...
def _showroom_uri(showroom_url: str) -> str:
...
def parse_showroom_details(showroom_url: str) -> dict:
    showroom_uri = _showroom_uri(showroom_url)
...
# -------------------------
def _get_products_page(showroom_uri: str, page: int, page_size: int = 36) -> dict:
...
def scrape_showroom(showroom_url: str):
    ...
    try:
        first_page = _get_products_page(showroom_uri, page=0)
    except Exception as e:
    ...
    for page in range(1, pages_count):
        for attempt in range(3):
            try:
                data = _get_products_page(showroom_uri, page=page)
